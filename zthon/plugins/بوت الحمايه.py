...
async def _(event):
    ...
    if input_str == "البوتات":
        ...
        return await edit_or_reply(event, "**⎉╎تـم قفـل {}  ✅ •**\n\n**⎉╎خاصيـة الطـرد والتحذيـر •**".format(input_str))
...
async def _(event):
   ...
   # All Rights Reserved for "Zed-Thon - ZelZal" "زلـزال الهيبـه"
    ...
    if input_str == "البوتات":
        ...
        return await edit_or_reply(event, "**⎉╎فتـحت** {} ** ✅ 𓆰•**".format(input_str))
    if input_str == "الدخول":
...
async def _(event):
    ...
    if event.user_joined: 
        ...
        if BOTLOG and is_ban_able:
            ban_reason_msg = await event.client.send_message(BOTLOG_CHATID,
                "**⎉╎سيـدي المـالك**\n\n**⎉╎قـام هـذا** [الشخـص](tg://user?id={})  \n**⎉╎بالانضمـام للمجمـوعـة**\n**⎉╎تم تحذيـر الشخـص وطـرده ..  ✓𓆰**".format(
                    zedy.id
                )
            )
...
async def _(event):
    ...
    if event.user_added:
        ...
        if BOTLOG and is_ban_able:
            ban_reason_msg = await event.client.send_message(BOTLOG_CHATID,
                "**⎉╎سيـدي المـالك**\n\n**⎉╎قـام هـذا** [الشخـص](tg://user?id={})  \n**⎉╎باضـافة بـوت للمجمـوعـة**\n**⎉╎تم تحذيـر الشخـص وطـرد البـوت ..  ✓𓆰**".format(
                    zelzal_by
                )
            )
    ...
    async for user in zed.client.iter_participants(zed.chat_id):
        if user.bot:
            try:
                ...
                del_a += 1
    if del_u > 0:
        del_status = f"**⎉╎تم طـرد  {del_u}  بـوت .. 🚮**"
    if del_a > 0:
