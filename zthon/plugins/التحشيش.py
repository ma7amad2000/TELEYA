...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𓏺𝙎𝙊𝙐𝙍𝘾𝞝 𝗭𝗧𝗵𝗼𝗻𝙃𝘼𝙔𝘼](t.me/HL_BG) 🗑♻️𓆪\n**𓍹━─━─━─━─𓏺𝙎𝙊𝙐𝙍𝘾𝙀 𝙃𝘼𝙔𝘼─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**| : 𖢿 فشن تحس هذا واحد من المطورين الاساسيين 😒😒  **")
    if user.id == 6195765774 or user.id == 6195765774 or user.id == 6195765774:
        return await edit_or_reply(mention, f"**| : 𖢿 حاايي هذاا الوسڪي ححبقلببي وصاحب السوࢪس  **")
...
ZelzalTHS_cmd = (
    "𓆩 [𓏺𝙎𝙊𝙐𝙍𝘾𝙀 𝙃𝘼𝙔𝘼 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/HL_BG) 𓆪\n\n"
...
async def zed(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**| : 𖢿 فشن تحس هذا واحد من المطورين الاساسيين 😒😒  **")
    if user.id == 6195765774 or user.id == 6195765774 or user.id == 6195765774:
        return await edit_or_reply(mention, f"**| : 𖢿 لكحاايي هذاا الوسڪي ححبقلببي وصاحب السوࢪس دي . . هـذا مطـور السـورس  **")
    zedth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**| : 𖢿 لك هـذا مطـور . . ممانقدرش نوصفه🙊💘 **")
    if user.id == 6195765774 or user.id == 6195765774 or user.id == 6195765774:
        return await edit_or_reply(mention, f"**| : 𖢿 حاايي هذاا الوسڪي ححبقلببي وصاحب السوࢪس 🙊💘 **")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def permalink(mention):
    ...
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**| : 𖢿 فشن تحس هذا واحد من المطورين الاساسيين 😒😒  **")
    if user.id == 6195765774 or user.id == 6195765774 or user.id == 6195765774:
        return await edit_or_reply(mention, f"**| : 𖢿 حاايي هذاا الوسڪي ححبقلببي وصاحب السوࢪس انت لي انهينك🙂  **")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
