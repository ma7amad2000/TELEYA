...
@zedub.zed_cmd(
    ...
    info={
        ...
        "الوصـف": "لـ إطفـاء الداينـو الخاص بتنصيبك بهيروكـو .. لا تستطيع اعاده التشغيل مرة اخرى عبر حسابك عليك الذهاب لحساب هيروكو واتباع الشرح التالي https://t.me/HL_BG",
...
async def _(event):
    "لـ إطفـاء البـوت"
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "#ايقــاف البــوت\n\n" "**- بـوت حياه فـي وضــع الايقــاف**")
    await edit_or_reply(event, "**| : 𖢿╎هِٰيٰ ثٖوَِꪆآِإَنَٖيٰ وَِꪆحٰٖآِنَٖڜٰڜٰتغٓلَٖ😻 ٖ 📟 ...**\n\n**| : 𖢿╎شغِّـلنـي يـدويًـا لاحقًــا**\n**| : 𖢿╎باتبـاع الشـرح** https://t.me/HL_BG")
...
@zedub.zed_cmd(
    pattern="ارقد( [0-9]+)?$",
    command=("ارقد", plugin_category),
    info={
        "header": "البـوت الخـاص بك سيتوقف موقتـاً .. حسب الثوانـي المدخلـه",
        "الاستخـدام": "{tr}ارقد <عـدد الثـواني>",
        "مثــال": "{tr}ارقد 60",
    },
)
async def _(event):
    ...
    if " " not in event.pattern_match.group(1):
        return await edit_or_reply(event, "**- اڪتب عدد الثواني لي تبي بوتك يرقد فيهم**\n**- مثــال :**\n`.ارقد 60`")
    ...
    sleep(counter)
    await event.edit("**| : 𖢿╎ٖجيتتتڪم🏃...**\n**| : 𖢿╎انا ٖ في وضـع التشغيـل ☑️**")
