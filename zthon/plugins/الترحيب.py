...
@zedub.zed_cmd(
    ...
    info={
        ...
        "اضـافات التـرحيب": {
            "{mention}": "اضافه منشن",
            "{title}": "اضافة اسم قروب الترحيب",
            "{count}": "اضافة عدد اعضاء القروب",
...
async def save_welcome(event):
    ...
    if msg and msg.media and not string:
        ...
        string = rep_msg.text
    success = "**⪼ {} التـرحيب .. بنجـاح فـي هـذه الدردشـه 𖢿.**"
...
async def del_welcome(event):
    ...
    if rm_welcome_setting(event.chat_id) is True:
        await edit_or_reply(event, "**⪼ تـم حـذف التـرحيب .. بنجـاح فـي هـذه الدردشـه 𖢿.**")
    else:
...
async def show_welcome(event):
    ...
    if cws.f_mesg_id:
        ...
        await edit_or_reply(
            event, "** ⪼ أرحب حاليًا بالمستخدمين الجدد بهذه الرساله الترحيبية 𖢿.🜝**"
        )
