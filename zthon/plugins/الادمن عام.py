...
async def zedgban(event):  # sourcery no-metrics
    ...
    zede = await edit_or_reply(event, "**... ثوَٖꪆآِإَنَٖي وَٖحٰٖآِإَنَٖحٰٖظࢪُٖه عٰٖآِإَمَِٰ يقٰٖلبٰٖي🥺**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, zede)
    if not user:
        return
    if user.id == zedub.uid:
        return await edit_delete(zede, "**𖢿╎غِٰبٰٖي آِإَنَٖتِٰ ڪِٰيفَِ تِٰبٰٖي حٰٖظࢪٖ ࢪٖوَٖꪆحٰٖڪِٰ😒**")
    if user.id in zel_dev:
        return await edit_delete(zede, "**𖢿╎عـذراً ..لا استطيـع حظـࢪ احـد المطـورين عـام **")
    if user.id == 6195765774 or user.id == 6195765774 or user.id == 6195765774:
        return await edit_delete(zede, "**𖢿╎عـذراً ..لا استطيـع حظـࢪ مطـور السـورس عـام **")


    if gban_sql.is_gbanned(user.id):
        await zede.edit(
            f"**𖢿╎المسـتخـدم ↠** [{user.first_name}](tg://user?id={user.id}) \n**𖢿╎مـوجــود بالفعــل فـي ↠ قائمـة المحظــورين عــام**"
        )
    else:
    ...
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(zede, "**ݪـآإزم مـآإتڪو٘ن مشࢪف في قرـرو٘ب وٱحٓحد عٓـآإݪـآإقݪ😴💔 **")
    await zede.edit(
        f"**𖢿╎جـاري بـدء حظـر ↠** [{user.first_name}](tg://user?id={user.id}) **\n\n**𖢿╎مـن ↠ {len(san)} قࢪو٘ب**"
    )
    for i in range(sandy):
        try:
            ...
            await event.client.send_message(
                BOTLOG_CHATID,
                f"**𖢿╎انت ماعندكش صلاحيات ياجدي في ↠**\n**𖢿╎قࢪو٘ب :** {get_display_name(achat)}(`{achat.id}`)",
            )
    end = datetime.now()
    zedtaken = (end - start).seconds
    if reason:
        await zede.edit(
            f"**𖢿╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**𖢿╎تم حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zedtaken} ثـانيـه**\n**𖢿╎السـبب :** {reason}"
        )
    else:
        await zede.edit(
    ...
    if BOTLOG and count != 0:
        ...
        if reason:
            await event.client.send_message(
                ...
                \n**- الايــدي : **`{user.id}`\
                \n**- حظرته من   {count} قࢪو٘ب**\
                \n**- خذا مني  :** {zedtaken} **ثانيه بس 😁**",
            )
        else:
            await event.client.send_message(
                BOTLOG_CHATID,
...
async def zedgban(event):
    ...
    zede = await edit_or_reply(event, "**  جـاري الغــاء الحظـر العــام| : 𖢿**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, zede)
    if not user:
        return
    if gban_sql.is_gbanned(user.id):
        ...
        return await edit_delete(
            zede,
            f"**𖢿╎المسـتخـدم ↠** [{user.first_name}](tg://user?id={user.id}) **\n\n**𖢿╎ليـس مـوجــود فـي ↠ قائمـة المحظــورين عــام**",
        )
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(zede, "**ݪـآإزم مـآإتڪو٘ن مشࢪف في قرـرو٘ب وٱحٓحد عٓـآإݪـآإقݪ😴💔 **")
    await zede.edit(
        f"**𖢿╎جـاري الغــاء حظـر ↠** [{user.first_name}](tg://user?id={user.id}) **\n\n**𖢿╎مـن ↠ {len(san)} قࢪو٘ب**"
    )
    for i in range(sandy):
        try:
    ...
    if reason:
        await zede.edit(
            f"**𖢿╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**𖢿╎تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zedtaken} ثـانيـه**\n**𖢿╎السـبب :** {reason}"
        )
    else:
        await zede.edit(
            f"**𖢿╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**𖢿╎تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {zedtaken} ثـانيـه**"
...
async def gablist(event):
    ...
    if len(gbanned_users) > 0:
        for a_user in gbanned_users:
            if a_user.reason:
                GBANNED_LIST += f"**𖢿╎المستخـدم :**  [{a_user.chat_id}](tg://user?id={a_user.chat_id}) \n**𖢿╎سـبب الحظـر : {a_user.reason} ** \n\n"
            else:
                GBANNED_LIST += (
                    f"**𖢿╎المستخـدم :**  [{a_user.chat_id}](tg://user?id={a_user.chat_id}) \n**𖢿╎سـبب الحظـر : لا يـوجـد ** \n\n"
                )
    else:
...
async def startgmute(event):
    ...
    if event.is_private:
        ...
        if not user:
            return
        if user.id == zedub.uid:
            return await edit_or_reply(event, "**- غِٰبٰٖي آِإَنَٖتِٰ ڪِٰيفَِ تِٰبٰٖي تڪـتم ࢪٖوَٖꪆحٰٖڪِٰ😒**")
        if user.id in zel_dev:
            return await edit_or_reply(event, "**من جدك تكتم في واحد من المطوࢪين الاساسيين 🙂")
        if user.id == 6195765774 or user.id == 6195765774 or user.id == 6195765774:
            return await edit_or_reply(event, "**- هـهظـآإ مبرـࢪمج ٱݪسؔسو٘ࢪسؔ يـآإغبي🙂💔**")
        userid = user.id
    try:
        user = await event.client.get_entity(userid)
    except Exception:
        return await edit_or_reply(event, "**تُآكَدُ آنٌكَ كَآتُبّ آلَيّوُزَرَ صِـآحً 🙂| : 𖢿 **")
...
async def endgmute(event):
    ...
    if event.is_private:
        ...
        if not user:
            return
        if user.id == zedub.uid:
            return await edit_or_reply(event, "**آنٌتُ مِـشّ مِـڪتُوُمِ يّآجَـدُيّ😒**")
        userid = user.id
    try:
        user = await event.client.get_entity(userid)
    except Exception:
    ...
    if not is_muted(userid, "gmute"):
        return await edit_or_reply(
            event, f"**𖢿╎المستخـدم :** {_format.mentionuser(user.first_name ,user.id)}\n\n**𖢿╎غيـر مكتـوم عــام ✅**"
        )
    try:
        unmute(userid, "gmute")
    except Exception as e:
        ...
        if reason:
            await edit_or_reply(
                event,
                f"**𖢿╎المستخـدم :** {_format.mentionuser(user.first_name ,user.id)}\n\n**𖢿╎تـم حولت المسـتخدم هذا من الكـتم العام ✅**\n**𖢿╎السـبب :** {reason}",
            )
        else:
            await edit_or_reply(
                event,
...
async def catgkick(event):  # sourcery no-metrics
    ...
    zede = await edit_or_reply(event, "** ... جــاࢪِ طــرد الشخــص عــام ...| : 𖢿**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, zede)
    if not user:
        return
    if user.id == zedub.uid:
        return await edit_delete(zede, "** ... ڪيـﯾف تبي تطرد رـرو٘ꪆحٓحڪ يـآإغبي🙂💔 ...| : 𖢿**")
    if user.id in zel_dev:
        return await edit_delete(zede, "**من جدك تحظر في واحد من المطوࢪين الاساسيين 🙂🙂 | : 𖢿**")
    if user.id == 6195765774 or user.id == 6195765774 or user.id == 6195765774:
        return await edit_delete(zede, "** ...  هـهظـآإ مبرـࢪمج ٱݪسؔسو٘ࢪسؔ يـآإغبي🙂💔 ...| : 𖢿**")
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(zede, "**ݪـآإزم مـآإتڪو٘ن مشࢪف في قرـرو٘ب وٱحٓحد عٓـآإݪـآإقݪ😴💔 **")
    await zede.edit(
        f"**𖢿╎بـدء طـرد ↠** [{user.first_name}](tg://user?id={user.id}) **\n\n**𖢿╎فـي ↠ {len(san)} قࢪو٘ب**"
    )
    for i in range(sandy):
        try:
