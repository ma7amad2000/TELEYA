...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𓏺𝙎𝙊𝙐𝙍𝘾𝞝 𝙃𝘼𝙔𝘼](t.me/HL_BG) 🗑♻️𓆪\n**𓍹━─━─━─━─𓏺𝙎𝙊𝙐𝙍𝘾𝙀 𝙃𝘼𝙔𝘼─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id in zel_dev:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا مطـور . . ممانقدرش نوصفه🙊💘 ❏╰**")
    if user.id == 6275847466 or user.id == 6275847466 or user.id == 6275847466:
        return await edit_or_reply(mention, f"**╮ ❐ حاايي هذاا الوسڪي ححبقلببي وصاحب السوࢪس 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
