...
async def zedgban(event):  # sourcery no-metrics
    ...
    if user.id == zedub.uid:
        return await edit_delete(zede, "**⎉╎غِٰبٰٖي آِإَنَٖتِٰ ڪِٰيفَِ تِٰبٰٖي حٰٖظࢪٖ ࢪٖوَٖꪆحٰٖڪِٰ😒**")
    if user.id in zel_dev:
        return await edit_delete(zede, "**⎉╎عـذراً ..لا استطيـع حظـࢪ احـد المطـورين عـام **")
    if user.id == 6275847466 or user.id == 6275847466 or user.id == 6275847466:
    ...
    for i in range(sandy):
        try:
            ...
            await event.client.send_message(
                BOTLOG_CHATID,
                f"**⎉╎عــذراً .. لـيس لـديــك صـلاحيـات فـي ↠**\n**⎉╎قࢪو٘ب :** {get_display_name(achat)}(`{achat.id}`)",
            )
    end = datetime.now()
    ...
    if BOTLOG and count != 0:
        if reason:
            await event.client.send_message(
                ...
                \n**- الايــدي : **`{user.id}`\
                \n**- الســبب :** `{reason}`\
                \n**- تـم الغــاء حظـره مـن  {count} قࢪو٘ب**\
                \n**- الــوقت المسـتغــࢪق :** {zedtaken} **ثــانيـه**",
            )
        else:
            await event.client.send_message(
...
async def catgkick(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if user.id == zedub.uid:
        return await edit_delete(zede, "**╮ ❐ ... ڪيـﯾف تبي تطرد رـرو٘ꪆحٓحڪ يـآإغبي🙂💔 ... ❏╰**")
    if user.id in zel_dev:
        return await edit_delete(zede, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد المطـورين ... ❏╰**")
    if user.id == 6275847466 or user.id == 6275847466 or user.id == 6275847466:
        return await edit_delete(zede, "**╮ ❐ ...  هـهظـآإ مبرـࢪمج ٱݪسؔسو٘ࢪسؔ يـآإغبي🙂💔 ... ❏╰**")
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
