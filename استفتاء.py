...
async def pollcreator(catpoll):
    ...
    if not string:
        options = Build_Poll(["- ايي 😊✌️", "- لاع 😏😕", "- مش عارف 🥱🙄"])
        try:
            ...
            await edit_or_reply(
                catpoll, "**⌔| : 𖢿 الاستفتاء المستخدم غير صالح (قد تكون المعلومات طويلة جدا).**"
            )
        except ForbiddenError:
            await edit_or_reply(catpoll, "**⌔| : 𖢿 هذه الدردشة تحظر استطلاعات الرأي. **")
        ...
        if len(catinput) > 2 and len(catinput) < 12:
            ...
            try:
                ...
                await edit_or_reply(
                    catpoll,
                    "**⌔| : 𖢿 الاستفتاء المستخدم غير صالح (قد تكون المعلومات طويلة جدا).**",
                )
            except ForbiddenError:
                await edit_or_reply(catpoll, "**⌔| : 𖢿 هذه الدردشة تحظر استطلاعات الرأي. **")
            except Exception as e:
            ...
            await edit_or_reply(
                catpoll,
                "**⌔| : 𖢿عـذراً عـزيـزي .. انت تكتب الامـر بشكـل خاطئ يجب عليك اعـادة كتابتـه بالشكـل التـالي :**\n\n`.استفتاء السؤال | الجواب الاول | الجواب الثاني`\n**⌔| : 𖢿لا تنسـى كتابـة الرمـز | بيـن كـل جـواب والثـاني**",
            )
