...
حقـوق للتـاريخ : @HL_BG
@bp_bo -  كتـابـة الملـف :  زلــزال الهيبــهت تعديل الوسڪي"""
...
ZelzalVP_cmd = (
    "𓆩 [𓏺𝙎𝙊𝙐𝙍𝘾𝞝 𝗭𝗧𝗵𝗼𝗻𝙃𝘼𝙔𝘼  𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر الفـارات](t.me/HL_BG) 𓆪\n\n"
    "** 𖢿╎قائـمه اوامر تغييـر فـارات الصـور بأمـر واحـد فقـط - لـ اول مـره ع سـورس زدثون حياه يوزر بـوت 🦾 :** \n\n"
    ...
    "** 𖢿╎قائـمه اوامر تغييـر كليشـة الايـدي :** \n\n"
    ...
    "** 𖢿╎قائـمه اوامر تغييـر بقيـة الفـارات بأمـر واحـد فقـط :** \n\n"
    ...
    "⪼ `.حذف المطورين`\n\n"
    "** 𖢿╎قائـمه اوامر تغييـر المنطقـة الزمنيـة للوقـت 🌐:** \n\n"
    "⪼ `.وقت العراق` \n\n"
    "⪼ `.وقت مصر` \n\n"
    "⪼ `.وقت الامارات` \n\n"
    "⪼ `.وقت ايران` \n\n"
    ...
    "\n𓆩 [𝙑𝙖𝙧𝙨 - قنـاة الفـارات](t.me/HL_BG) 𓆪"
)
...
async def variable(event):
    ...
    vinfo = reply.text
    zed = await edit_or_reply(event, "** 𖢿╎جـاري إضـافة الفـار إلــى بـوتك ...**")
    ...
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        ...
        if gvarstatus("ALIVE_TEMPLATE") is None:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n** 𖢿╎الكليشـة الجـديده** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.فحص` **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        else:
            await zed.edit("** 𖢿╎تم اضـافـة {} بنجـاح ☑️**\n** 𖢿╎الكليشـة المضـافه** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.فحص` **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        ...
        if gvarstatus("pmpermit_txt") is None:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n** 𖢿╎الكليشـة الجـديده** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.الحمايه تفعيل` **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        else:
            await zed.edit("** 𖢿╎تم اضـافـة {} بنجـاح ☑️**\n** 𖢿╎الكليشـة المضـافه** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.الحمايه تفعيل` **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        ...
        if gvarstatus("START_TEXT") is None:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n** 𖢿╎الكليشـة الجـديده** \n {} \n\n** 𖢿╎الان قـم بـ الذهـاب لبوتك المسـاعد من حساب آخر ↶** ودز ستارت **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        else:
            await zed.edit("** 𖢿╎تم اضـافـة {} بنجـاح ☑️**\n** 𖢿╎الكليشـة المضـافه** \n {} \n\n** 𖢿╎الان قـم بـ الذهـاب لبوتك المسـاعد من حساب آخر ↶** ودز ستارت **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        ...
        if gvarstatus("CUSTOM_ALIVE_EMZED") is None:
            ...
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n** 𖢿╎الـرمـز الجـديـد** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.الاسم تلقائي` **لـ التحقـق مـن الـرمز . .**".format(input_str, vinfo))
        else:
            addgvar(variable, vinfo)
            await zed.edit("** 𖢿╎تم اضـافـة {} بنجـاح ☑️**\n** 𖢿╎الـرمـز المضـاف** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.الاسم تلقائي` **لـ التحقـق مـن الـرمز . .**".format(input_str, vinfo))
        ...
        if gvarstatus("DEFAULT_BIO") is None:
            addgvar(variable, vinfo)
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n** 𖢿╎البـايـو الجـديـد** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.البايو تلقائي` **لـ التحقـق مـن البايـو . .**".format(input_str, vinfo))
        else:
            addgvar(variable, vinfo)
            await zed.edit("** 𖢿╎تم اضـافه {} بنجـاح ☑️**\n** 𖢿╎البـايـو المضـاف** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.البايو تلقائي` **لـ التحقـق مـن البايـو . .**".format(input_str, vinfo))
    elif input_str == "كاشف الاباحي" or input_str == "كشف الاباحي":
        variable = "DEEP_API"
        await asyncio.sleep(1.5)
        if gvarstatus("DEEP_API") is None:
            addgvar(variable, vinfo)
            await zed.edit("** 𖢿╎تم تغييـر توكـن {} بنجـاح ☑️**\n** 𖢿╎التوكـن الجـديـد** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.قفل الاباحي` **لـ تفعيـل كاشـف الاباحي . .**".format(input_str, vinfo))
        else:
            addgvar(variable, vinfo)
            await zed.edit("** 𖢿╎تم إضافـة توكـن {} بنجـاح ☑️**\n** 𖢿╎التوكـن المضـاف** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.قفل الاباحي` **لـ تفعيـل كاشـف الاباحي . .**".format(input_str, vinfo))
        ...
        if gvarstatus("CUSTOM_ALIVE_EMOJI") is None:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n\n** 𖢿╎المتغيـر : ↶**\n `{}`\n** 𖢿╎ارسـل الان** `.ايدي`".format(input_str, vinfo))
        else:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n\n** 𖢿╎المتغيـر : ↶**\n `{}`\n** 𖢿╎ارسـل الان** `.ايدي`".format(input_str, vinfo))
        ...
        if gvarstatus("Custom_Pm_Channel") is None:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n\n** 𖢿╎المتغيـر : ↶**\n `{}`\n** 𖢿╎ارسـل الان** `.اشتراك خاص`".format(input_str, vinfo))
        else:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n\n** 𖢿╎المتغيـر : ↶**\n `{}`\n** 𖢿╎ارسـل الان** `.اشتراك خاص`".format(input_str, vinfo))
        ...
        if gvarstatus("Custom_G_Channel") is None:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n\n** 𖢿╎المتغيـر : ↶**\n `{}`\n** 𖢿╎ارسـل الان** `.اشتراك كروب`".format(input_str, vinfo))
        else:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n\n** 𖢿╎المتغيـر : ↶**\n `{}`\n** 𖢿╎ارسـل الان** `.اشتراك كروب`".format(input_str, vinfo))
        ...
        if gvarstatus("ZAGL_Zed") is None:
            addgvar(variable, vinfo)
            await zed.edit("** 𖢿╎تم اضافـة {} بنجـاح ☑️**\n** 𖢿╎اليـوزرات المضـافة** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.زاجل` **بالـرد ع نـص او ميديـا بنـص . .**".format(input_str, vinfo))
        else:
            addgvar(variable, vinfo)
            await zed.edit("** 𖢿╎تم اضافـة {} بنجـاح ☑️**\n** 𖢿╎اليـوزرات المضـافة** \n {} \n\n** 𖢿╎الان قـم بـ ارسـال الامـر ↶** `.زاجل` **بالـرد ع نـص او ميديـا بنـص . .**".format(input_str, vinfo))
    elif input_str == "اسم المستخدم" or input_str == "الاسم":
        if Config.HEROKU_API_KEY is None:
            return await ed(event, " 𖢿╎اضبط Var المطلوب في Heroku على وظيفة هذا بشكل طبيعي `HEROKU_API_KEY` اذا كنت لاتعلم اين يوجد فقط اذهب الى حسابك في هيروكو ثم الى الاعدادات ستجده بالاسفل انسخه ودخله في الفار. ")

        if Config.HEROKU_APP_NAME is not None:
            app = Heroku.app(Config.HEROKU_APP_NAME)
        else:
            return await ed(event, " 𖢿╎اضبط Var المطلوب في Heroku على وظيفة هذا بشكل طبيعي `HEROKU_APP_NAME` اسم التطبيق اذا كنت لاتعلم.")
        heroku_var = app.config()
        variable = "ALIVE_NAME"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("** 𖢿╎تم تغييـر {} بنجـاح ☑️**\n** 𖢿╎المتغيـر : ↶**\n `{}` \n** 𖢿╎يتم الان اعـادة تشغيـل بـوت سبـارك يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        else:
            await zed.edit("** 𖢿╎تم اضافـة {} بنجـاح ☑️** \n** 𖢿╎المضاف اليه :**\n `{}` \n** 𖢿╎يتم الان اعـادة تشغيـل بـوت سبـارك يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, vinfo))
        ...
        if vinfo.isdigit():
            ...
            return await zed.edit("** 𖢿╎خطـأ .. قم بالـرد ع رقـم فقـط ؟!**")
        heroku_var[variable] = vinfo
        ...
        if input_str:
            return await zed.edit("** 𖢿╎عـذࢪاً .. لايوجـد هنالك فـار بإسـم {} ؟!.. ارسـل (.اوامر الفارات) لـعرض قائمـة الفـارات**".format(input_str))

        return await edit_or_reply(event, "** 𖢿╎عـذࢪاً .. لايوجـد هنالك فـار بإسـم {} ؟!.. ارسـل (.اوامر الفارات) لـعرض قائمـة الفـارات**".format(input_str))
...
async def variable(event):
    if Config.HEROKU_API_KEY is None:
        return await ed(
            event,
            " 𖢿╎اضبط Var المطلوب في Heroku على وظيفة هذا بشكل طبيعي `HEROKU_API_KEY` اذا كنت لاتعلم اين يوجد فقط اذهب الى حسابك في هيروكو ثم الى الاعدادات ستجده بالاسفل انسخه ودخله في الفار. ",
        )
    if Config.HEROKU_APP_NAME is not None:
        app = Heroku.app(Config.HEROKU_APP_NAME)
    else:
        return await ed(
            event,
            " 𖢿╎اضبط Var المطلوب في Heroku على وظيفة هذا بشكل طبيعي `HEROKU_APP_NAME` اسم التطبيق اذا كنت لاتعلم.",
        )
    input_str = event.text[5:]
    heroku_var = app.config()
    zed = await edit_or_reply(event, "** 𖢿╎جـاري حـذف الفـار مـن بـوتك 🚮...**")
    # All Rights Reserved for "محمد - Tepthon"
    if input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        variable = gvarstatus("ALIVE_TEMPLATE")
        await asyncio.sleep(1.5)
        if gvarstatus("ALIVE_TEMPLATE") is None:
        	return await zed.edit("** 𖢿╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("** 𖢿╎تم حـذف {} بنجـاح ☑️**\n** 𖢿╎المتغيـر المحـذوف : ↶**\n `{}` \n** 𖢿╎يتم الان اعـادة تشغيـل بـوت سبـارك يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, variable))
        ...
        if gvarstatus("ALIVE_PIC") is None:
        	return await zed.edit("** 𖢿╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("** 𖢿╎تم حـذف فـار {} . . بنجـاح ☑️**".format(input_str))
        delgvar("ALIVE_PIC")
        ...
        if variable not in heroku_var:
        	return await zed.edit("** 𖢿╎عـذࢪاً عـزيـزي .. انت لـم تقـم باضـافـة فـار {} اصـلاً...**".format(input_str))
        await zed.edit("** 𖢿╎تم حـذف {} بنجـاح ☑️**\n** 𖢿╎المتغيـر المحـذوف : ↶**\n `{}` \n** 𖢿╎يتم الان اعـادة تشغيـل بـوت سبـارك يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str, heroku_var[variable]))
...
async def custom_zed(event):
    input_str = event.text[5:]
    zed = await edit_or_reply(event, "** 𖢿╎جــاري جلـب معلـومـات الفــار 🛂. . .**")
    if (input_str == "كليشة الحماية" or input_str == "كليشة الحمايه" or input_str == "كليشه الحماية" or input_str == "كليشه الحمايه"):
        variable = gvarstatus("pmpermit_txt")
        if variable is None:
            await zed.edit("** 𖢿╎فـار كليشـة الحمايـة غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الكليشـة استخـدم الامـر : ↶**\n `.اضف كليشة الحماية` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
            
    elif input_str == "كليشة الفحص" or input_str == "كليشه الفحص":
        variable = gvarstatus("ALIVE_TEMPLATE")
        if variable is None:
            await zed.edit("** 𖢿╎فـار كليشـة الفحص غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الكليشـة استخـدم الامـر : ↶**\n `.اضف كليشة الفحص` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = gvarstatus("START_TEXT")
        if variable is None:
            await zed.edit("** 𖢿╎فـار كليشـة البـوت غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الكليشـة استخـدم الامـر : ↶**\n `.اضف كليشة البوت` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = gvarstatus("pmblock")
        if variable is None:
            await zed.edit("** 𖢿╎فـار كليشـة الحظـر غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الكليشـة استخـدم الامـر : ↶**\n `.اضف كليشة الحظر` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("** 𖢿╎فـار رمـز الوقتـي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الرمـز استخـدم الامـر : ↶**\n `.اضف رمز الوقتي` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = gvarstatus("DEEP_API")
        if variable is None:
            await zed.edit("** 𖢿╎فـار كشـف الاباحي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الكـود استخـدم الامـر : ↶**\n `.اضف كاشف الاباحي` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("** 𖢿╎فـار البايـو الوقتـي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع نـص استخـدم الامـر : ↶**\n `.اضف البايو` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "ALIVE_NAME"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار اسـم المستخـدم غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الاسم استخـدم الامـر : ↶**\n `.اضف اسم المستخدم` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "OWNER_ID"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار ايـدي الحسـاب غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الايـدي فقـط استخـدم الامـر : ↶**\n `.اضف ايدي الحساب` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "COMMAND_HAND_LER"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار نقطـة الاوامـر غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الرمـز فقـط استخـدم الامـر : ↶**\n `.اضف نقطة الاوامر` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "TG_BOT_TOKEN"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار توكـن البـوت غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع التوكـن فقـط استخـدم الامـر : ↶**\n `.اضف التوكن` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "TG_BOT_USERNAME"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار معرف البوت غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع المعرف استخـدم الامـر : ↶**\n `.اضف معرف البوت` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "UPSTREAM_REPO"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار الريبـو غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع رابط السورس الرسمي استخـدم الامـر : ↶**\n `.اضف الريبو` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "AUTONAME"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار الاسـم التلقائي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الاسم استخـدم الامـر : ↶**\n `.اضف اسمي التلقائي` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("** 𖢿╎فـار صـورة الحمايـة غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع صـورة فقـط استخـدم الامـر : ↶**\n `.اضف صورة الحماية` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = gvarstatus("DIGITAL_PIC")
        if variable is None:
            await zed.edit("** 𖢿╎فـار صـورة الوقتـي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع صـورة فقـط استخـدم الامـر : ↶**\n `.اضف صورة الوقتي` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("** 𖢿╎فـار صـورة الفحص غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع صـورة فقـط استخـدم الامـر : ↶**\n `.اضف صورة الفحص` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("** 𖢿╎فـار صـورة ستـارت البـوت غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع صـورة فقـط استخـدم الامـر : ↶**\n `.اضف صورة البوت` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("** 𖢿╎فـار صـورة الاوامـر غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع صـورة فقـط استخـدم الامـر : ↶**\n `.اضف صورة الاوامر` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = gvarstatus("ALIVE_PIC")
        if variable is None:
            await zed.edit("** 𖢿╎فـار صـورة السـورس غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع صـورة فقـط استخـدم الامـر : ↶**\n `.اضف صورة السورس` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "ZI_FN"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار زخرفـة الاسـم الوقتي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته فقـط استخـدم الامـر : ↶**\n `.الوقتي 1` إلــى `.الوقتي 14` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار رسـائل الحمايـة غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته فقـط استخـدم الامـر : ↶**\n `.اضف رسائل الحماية` بالـرد ع عـدد فقـط \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if variable is None:
            await zed.edit("** 𖢿╎فـار زخرفـة الصـورة الوقتيـة غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته فقـط استخـدم الامـر : ↶**\n `.وقتي 1` إلــى `.وقتي 17` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "TZ"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار المنطقـه الزمنيـه غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته فقـط استخـدم الامـر : ↶**\n `.وقت` واسـم الدولـة \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = gvarstatus("CUSTOM_ALIVE_EMOJI")
        if variable is None:
            await zed.edit("** 𖢿╎فـار ايموجي/رمز الايدي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الرمـز استخـدم الامـر : ↶**\n `.اضف رمز الايدي` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = gvarstatus("CUSTOM_ALIVE_TEXT")
        if variable is None:
            await zed.edit("** 𖢿╎فـار نص عنـوان كليشـة الايـدي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الرمـز استخـدم الامـر : ↶**\n `.اضف عنوان الايدي` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = gvarstatus("CUSTOM_ALIVE_FONT")
        if variable is None:
            await zed.edit("** 𖢿╎فـار خطـوط كليشـة الايـدي غيـر موجـود ❌**\n** 𖢿╎لـ اضـافته بالـرد ع الرمـز استخـدم الامـر : ↶**\n `.اضف خطوط الايدي` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        if gvarstatus("Z_AK") is None:
            await zed.edit("** 𖢿╎المتغيـر غيـر موجـود ❌**\n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎المتغيـر {} موجـود ☑️**\n** 𖢿╎قيمـة المتغيـر : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
        ...
        variable = "SPAMWATCH_API"
        if variable not in heroku_var:
            await zed.edit("** 𖢿╎فـار توكـن المكـافح غيـر موجـود ❌**\n\n** 𖢿╎قنـاة السـورس : @HL_BG**")
        else:
            await zed.edit("** 𖢿╎الفـار {} موجـود ☑️**\n** 𖢿╎قيمـة الفـار : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, variable))
...
async def variable(event):
    ...
    viran = "Asia/Tehran"
    heroku_var = app.config()
    zed = await edit_or_reply(event, "** 𖢿╎جـاري إعـداد المنطقـه الزمنيـه لـ تيبثـــون 🌐...**")
    # All Rights Reserved for "محمد - Tepthon"
    if input_str == "العراق" or input_str == "اليمن" or input_str == "سوريا" or input_str == "السعودية" or input_str == "لبنان" or input_str == "الاردن":
        variable = "TZ"
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("** 𖢿╎تم تغييـر المنطقـه الزمنيـه .. بنجـاح ☑️**\n** 𖢿╎المتغير : ↶**\n دولـة `{}` \n** 𖢿╎يتم الان اعـادة تشغيـل بـوت سبـارك يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
        else:
            await zed.edit("** 𖢿╎تم اضـافـة المنطقـه الزمنيـه .. بنجـاح ☑️**\n** 𖢿╎المضـاف اليـه : ↶**\n دولـة `{}` \n** 𖢿╎يتم الان اعـادة تشغيـل بـوت سبـارك يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(input_str))
...
async def _(malatha):
    if malatha.fwd_from:
        return
    zed = await edit_or_reply(malatha, "** 𖢿╎جـاري إضـافة فـار الصـورة إلــى بـوتك ...**")
    if not os.path.isdir(Config.TEMP_DIR):
        os.makedirs(Config.TEMP_DIR)
        #     if BOTLOG:
        await malatha.client.send_message(
            BOTLOG_CHATID,
            "** 𖢿╎تم إنشاء حساب Telegraph جديد {} للدورة الحالية‌‌** \n** 𖢿╎لا تعطي عنوان url هذا لأي شخص**".format(
                auth_url
            ),
        )
    ...
    if malatha.reply_to_msg_id:
        ...
        if input_str in ["الحماية", "الحمايه"]:
            ...
            try:
                ...
                await zed.edit("** 𖢿╎تم تغييـر صـورة {} .. بنجـاح ☑️**\n** 𖢿╎المتغيـر : ↶**\n `{}` \n\n** 𖢿╎قنـاة السـورس : @HL_BG**".format(input_str, vinfo))
        ...
        await zed.edit(
            "** 𖢿╎بالـرد على صـورة لتعييـن الفـار ...**",
        )
