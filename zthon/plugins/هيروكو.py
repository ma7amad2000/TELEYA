...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        ...
        if variable in heroku_var:
            await zed.edit("**⌔∮ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت حيــاه يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "جلب":
        ...
        await asyncio.sleep(1.5)
        if variable in heroku_var:
            await zed.edit("**⌔∮ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بــوت حيــاه يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
            await zed.edit("**⌔∮ تم اضافه** `{}` **:** \n**- المضاف اليه :** `{}` \n**يتم الان اعـادة تشغيـل بـوت حيــاه يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        heroku_var[variable] = value
    elif exe == "حذف":
        ...
        await zed.edit(f"**⌔∮** `{variable}`  **تم حذفه بنجاح. \n**يتم الان اعـادة تشغيـل بـوت حيــاه يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        del heroku_var[variable]
