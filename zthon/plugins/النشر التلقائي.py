...
ZelzalNSH_cmd = (
    "𓆩 [𓏺𝙎𝙊𝙐𝙍𝘾𝞝 𝗭𝗧𝗵𝗼𝗻𝙃𝘼𝙔𝘼 - اوامـر النشـر التلقـائي](t.me/HL_BG) 𓆪\n\n"
...
async def _(event):
    if (event.is_private or event.is_group):
        return await edit_or_reply(event, "**𖢿╎عـذراً .. النشر التلقائي خـاص بالقنـوات فقـط\n𖢿╎قم باستخـدام الامـر داخـل القنـاة الهـدف**")
    if input_str := event.pattern_match.group(2):
        try:
            zch = await event.client.get_entity(input_str)
        except Exception as e:
            return await edit_delete(event, "**𖢿╎عـذراً .. معـرف/ايـدي القنـاة غيـر صـالح**\n**𖢿╎الرجـاء التـأكـد مـن المعـرف/الايـدي**", 5)
        try:
            if is_post(zch.id , event.chat_id):
                return await edit_or_reply(event, "**𖢿╎النشـر التلقـائي مفعـل مسبقـاً ✓**")
            if zch.first_name:
                await asyncio.sleep(1.5)
                add_post(zch.id, event.chat_id)
                await edit_or_reply(event, "**𖢿╎تم تفعيـل النشـر التلقـائي من القنـاة .. بنجـاح ✓**")
        except Exception:
            try:
                if is_post(zch.id , event.chat_id):
                    return await edit_or_reply(event, "**𖢿╎النشـر التلقـائي مفعـل مسبقـاً ✓**")
                if zch.title:
                    await asyncio.sleep(1.5)
                    add_post(zch.id, event.chat_id)
                    return await edit_or_reply(event, "**𖢿╎تم تفعيـل النشـر التلقـائي من القنـاة .. بنجـاح ✓**")
            except Exception as e:
                LOGS.info(str(e))
        await edit_or_reply(event, "**𖢿╎عـذراً .. معـرف/ايـدي القنـاة غيـر صـالح**\n**𖢿╎الرجـاء التـأكـد مـن المعـرف/الايـدي**")
    ...
    if input_str := event.pattern_match.group(2):
        ...
        try:
            if not is_post(zch.id, event.chat_id):
                return await edit_or_reply(event, "**𖢿╎عـذراً .. النشـر التلقـائي غير مفعـل من اساسـاً ؟!**")
            if zch.first_name:
                await asyncio.sleep(1.5)
                remove_post(zch.id, event.chat_id)
                await edit_or_reply(event, "**𖢿╎تم تعطيـل النشر التلقـائي هنـا .. بنجـاح ✓**")
        except Exception:
            try:
                if not is_post(zch.id, event.chat_id):
                    return await edit_or_reply(event, "**𖢿╎عـذراً .. النشـر التلقـائي غير مفعـل من اساسـاً ؟!**")
                if zch.title:
                    await asyncio.sleep(1.5)
                    remove_post(zch.id, event.chat_id)
                    return await edit_or_reply(event, "**𖢿╎تم تعطيـل النشر التلقـائي هنـا .. بنجـاح ✓**")
            except Exception as e:
                LOGS.info(str(e))
        await edit_or_reply(event, "**𖢿╎عـذراً .. معـرف/ايـدي القنـاة غيـر صـالح**\n**𖢿╎الرجـاء التـأكـد مـن المعـرف/الايـدي**")
