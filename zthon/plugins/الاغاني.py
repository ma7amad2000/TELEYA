...
#                                                             𝙕𝙏𝙝𝙤𝙣
# =========================================================== #
SONG_SEARCH_STRING = "<b>𖢿╮ آِإَسُتِٰنَٖآِإَ آِإَنَٖدوَٖꪆࢪٖلڪِٰ فَِآِإَلمَِٰطلوَٖبٰٖ يآِإَڂِٰيي...🔥♥╰</b>"
SONG_NOT_FOUND = "<b>𖢿╎آِإَسُفَِ مَِٰآِإَلقٰٖيتِٰڜ لي تِٰبٰٖيُه 🥺💔 .. جرب البحث باستخـدام الامـر (.اغنيه)</b>"
SONG_SENDING_STRING = "<b>𖢿╮ جـارِ تحميـل الاغنيـٓه... 🔥♥╰</b>"
# =========================================================== #
#                                                             𝙕𝙏𝙝𝙤𝙣
# =========================================================== #
...
async def song(event):
    ...
    if event.pattern_match.group(2):
        ...
        return await edit_or_reply(event, "**𖢿╎قم باضافـة الاغنيـه للامـر .. بحث + اسـم الاغنيـه**")
    zed = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    zedevent = await edit_or_reply(event, "**𖢿╮ آِإَسُتِٰنَٖآِإَ آِإَنَٖدوَٖꪆࢪٖلڪِٰ فَِآِإَلمَِٰطلوَٖبٰٖ يآِإَڂِٰيي...🔥♥╰**")
    video_link = await yt_search(str(query))
    if not url(video_link):
        return await zedevent.edit(
            f"**𖢿╎ .. آِإَسُفَِ مَِٰآِإَلقٰٖيتِٰڜ لي تِٰبٰٖيُه 🥺💔** {query}"
        )
...
async def vsong(event):
    ...
    if event.pattern_match.group(1):
        ...
        return await edit_or_reply(event, "**𖢿╎قم باضافـة الاغنيـه للامـر .. فيديو + اسـم الفيديـو**")
    zed = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    zedevent = await edit_or_reply(event, "**𖢿╮ جـارِ البحث ؏ـن الفيديـو... 🔥♥╰**")
    video_link = await yt_search(str(query))
    if not url(video_link):
        return await zedevent.edit(
            f"**𖢿╎ .. آِإَسُفَِ مَِٰآِإَلقٰٖيتِٰڜ لي تِٰبٰٖيُه 🥺💔** {query}"
...
async def shazamcmd(event):
    ...
    await event.client.send_file(
        event.chat_id,
        file,
        caption=f"<b>𖢿╎ المقطـع الصـوتي :</b> <code>{title}</code>\n<b>𖢿╎ الرابـط : <a href = {slink}/1>YouTube</a></b>",
        reply_to=reply,
        parse_mode="html",
    )
    await zedevent.delete()
