...
async def log_tagged_messages(event):
    ...
    resalt = f"#لي دارولك اشاره\n\n<b>𖢿┊الـقــࢪوب : </b><code>{hmm.title}</code>"
    if full is not None:
        resalt += (
            f"\n\n<b>𖢿┊المـرسـل : </b> {_format.htmlmentionuser(full.first_name , full.id)}"
        )
    if messaget is not None:
        resalt += f"\n\n<b>𖢿┊رسـالـة ميـديـا : </b><code>{messaget}</code>"
    else:
        resalt += f"\n\n<b>𖢿┊الرســالـه : </b>{event.message.message}"
    resalt += f"\n\n<b>𖢿┊رابـط الرسـاله : </b><a href = 'https://t.me/c/{hmm.id}/{event.message.id}'> link</a>"
...
async def log(log_text):
    ...
    if BOTLOG:
        if log_text.reply_to_msg_id:
            ...
            await log_text.edit("**𖢿┊بالــرد على اي رسـاله لحفظهـا في كـروب التخــزين**")
            return
        await log_text.edit("**𖢿┊تـم الحفـظ في كـروب التخـزين .. بنجـاح ✓**")
    else:
        await log_text.edit("**𖢿┊عـذراً .. هـذا الامـر يتطلـب تفعيـل فـار التخـزين اولاً**")
...
async def set_no_log_p_m(event):
    ...
    if Config.PM_LOGGER_GROUP_ID != -100:
        ...
        if no_log_pms_sql.is_approved(chat.id):
            ...
            await edit_delete(
                event, "**𖢿┊تـم تفعيـل التخـزين لهـذه الدردشـه .. بنجـاح ✓**", 5
            )
...
async def set_no_log_p_m(event):
    ...
    if Config.PM_LOGGER_GROUP_ID != -100:
        ...
        if not no_log_pms_sql.is_approved(chat.id):
            ...
            await edit_delete(
                event, "**𖢿┊تـم تعطيـل التخـزين لهـذه الدردشـه .. بنجـاح ✓**", 5
            )
