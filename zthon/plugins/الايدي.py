...
async def _(event):
    ...
    if input_str := event.pattern_match.group(2):
        ...
        try:
            if p.first_name:
                return await edit_or_reply(
                    event, f"**𖢿╎ايـدي المستخـدم**  `{input_str}` **هـو** `{p.id}`"
                )
        except Exception:
            try:
                if p.title:
                    return await edit_or_reply(
                        event, f"**𖢿╎ايـدي المستخـدم**  `{p.title}` **هـو** `{p.id}`"
                    )
            except Exception as e:
                LOGS.info(str(e))
        await edit_or_reply(event, "**𖢿╎أدخل إما اسم مستخدم أو الرد على المستخدم**")
        ...
        if r_msg.media:
            ...
            await edit_or_reply(
                event,
                f"**𖢿╎ايـدي الدردشـه : **`{event.chat_id}`\n\n**𖢿╎ايـدي المستخـدم : **`{r_msg.sender_id}`\n\n**𖢿╎ايـدي الميديـا : **`{bot_api_file_id}`",
            )
        ...
        await edit_or_reply(event, f"**𖢿╎ايـدي الدردشـه : **`{event.chat_id}`")
