...
ZelzalPH_cmd = (
    "𓆩 [𓏺𝙎𝙊𝙐𝙍𝘾𝞝 𝗭𝗧𝗵𝗼𝗻𝙃𝘼𝙔𝘼 𝗖𝗼𝗻𝗳𝗶𝗴 📲 - كـاشـف الارقـام العربيــة](t.me/HL_BG) 𓆪\n\n"
    "**⪼ الامــر ↵**\n\n"
...
async def _(event):
    ...
    chat = "@Zelzalybot"
    zzzzl1l = await edit_or_reply(event, "**╮•𖢿 جـارِ الكـشف ؏ــن الـرقـم  📲 ⌭ . . .**")
    async with event.client.conversation(chat) as conv:
        try:
            response = conv.wait_event(
                events.NewMessage(incoming=True, from_users=6275847466)
            )
            await event.client.send_message(chat, "{}".format(input_str))
            response = await response
            await event.client.send_read_acknowledge(conv.chat_id)
        except YouBlockedUserError:
            await zzzzl1l.edit("**╮•𖢿 تحـقق من انـك لم تقـم بحظر البوت @Zelzalybot .. ثم اعـد استخدام الامـر ...🤖♥️**")
            return
        if response.text.startswith("I can't find that"):
            await zzzzl1l.edit("**╮•𖢿 عذرًا .. لـم استطـع ايجـاد المطلـوب ☹️💔**")
        else:
