    ...
    if input_str := event.pattern_match.group(2):
        ...
        try:
            if p.first_name:
                ...
                return await edit_or_reply(
                    event, f"**✧╎تم إضافة قناة الاشتراك الاجباري للقروب .. بنجـاح ☑️**\n\n**✧╎يوزر القناة : ↶** `{input_str}`\n**✧╎ايدي القناة : ↶** `{p.id}`\n\n**✧╎ارسـل الآن** `.اشتراك قروب`"
                )
        except Exception:
            try:
                if p.title:
        ...
        if r_msg.media:
            ...
            await edit_or_reply(
                event,
                f"**✧╎تم إضافة قناة الاشتراك الاجباري للقروب .. بنجـاح ☑️**\n\n**✧╎ايدي القناة : ↶** `{event.chat_id}`\n\n**✧╎ارسـل الآن** `.اشتراك قروب`",
            )
...
async def supc (event):
    ...
    if cc == "قروب" or cc == "جروب" or  cc == "قروب" or  cc == "مجموعة" or  cc == "مجموعه" or cc == "القروب" or cc == "اشتراك القروب":
        ...
        if gvarstatus ("sub_group"):
            delgvar ("sub_group")
            return await edit_delete(event, "**✧╎تم الغاء الاشتراك الاجباري للقروب .. بنجـاح ✓**")
